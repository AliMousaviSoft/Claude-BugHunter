...
- **How to disprove:** Find an input that *does* reach output unencoded (a header, a different parameter, a cookie). If every payload route gets encoded on output, it's a hardened app, not a bypassable one. Lesson reference: redacted-portal.ch — request validator blocks `<` before storage; encoding bypasses do not help.
